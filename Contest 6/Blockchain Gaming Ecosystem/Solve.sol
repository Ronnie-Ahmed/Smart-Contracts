...
contract BlockchainGamingEcosystem {
    ...
    function registerPlayer(string calldata userName) public {
         ...
         if(msg.sender==contractOwner || addressToPlayer[msg.sender].isPlayerRegistered || isUserNameTaken[userName])revert();
       
      
        bytes calldata strBytes = bytes(userName);

        assembly{
            if lt(strBytes.length,3){revert(0,0)}
        }

        addressToPlayer[msg.sender]=Player(msg.sender,userName,1000,0,true);
    ...
    function createGame(string calldata gameName, uint256 gameID) public  {
        ...
        if(isGameNameTaken[gameName] || gameToId[gameID].isGameOn || msg.sender!=contractOwner)revert();
unchecked{
    ...
    function removeGame(uint256 gameID) public  {
        if(msg.sender!=contractOwner || !gameToId[ uint8(gameID)].isGameOn )revert();
unchecked{
        uint8[] storage nftIds = gameNftIds[ gameID];
            ...
            addressToPlayer[owner].PlayerCredits+=itToNFT[nftIds[i]].assetPrice;
    ...
    function buyAsset(uint256 gameID) public  {
       unchecked{
        ...
        if(addressToPlayer[msg.sender].isPlayerRegistered==false || !gameToId[ gameID].isGameOn || addressToPlayer[msg.sender].PlayerCredits < currentPrice )revert();
    ...
    function sellAsset(uint256 tokenID) public  {
         if(addressToPlayer[msg.sender].isPlayerRegistered==false || itToNFT[uint8(tokenID)].assetOwner != msg.sender || itToNFT[tokenID].isNftIdExist==false) revert();
unchecked{
        uint8 gameid = itToNFT[tokenID].gameId;
        
        addressToPlayer[msg.sender].PlayerCredits+= gameToId[gameid].currentAssetPrice;
    ...
    function transferAsset(uint256 tokenID, address to) public  {
        ...
        if(itToNFT[tokenID].assetOwner != msg.sender || to == msg.sender || addressToPlayer[to].isPlayerRegistered==false || addressToPlayer[msg.sender].isPlayerRegistered==false || !itToNFT[tokenID].isNftIdExist )revert();
        unchecked{ 
        IGamingEcosystemNFT(nftContractAddress).transferNFT(tokenID, msg.sender, to);
        addressToPlayer[msg.sender].NFTcount-=1;
    ...
    {
        require(
           addressToPlayer[msg.sender].isPlayerRegistered || msg.sender == contractOwner,
            "Not a Registered Player"
        );
        require(addressToPlayer[playerAddress].isPlayerRegistered, "Transaction Failed");
    ...
    function viewAsset(uint256 tokenID)
        public
        view
        returns (address owner, string memory gameName, uint price)
    {
        if(!addressToPlayer[msg.sender].isPlayerRegistered && msg.sender != contractOwner)revert();
